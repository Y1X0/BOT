...
async function loadOverview(){ const c=document.getElementById('content'); c.innerHTML='<p class="muted" style="padding:16px">جاري التحميل...</p>';
  ...
  c.innerHTML='<div class="kpi-grid">'
    ...
    +'<div class="row"><select id="bcTarget"><option value="all">الكل (جروبات + قنوات)</option><option value="groups">الجروبات فقط</option><option value="channels">القنوات فقط</option><option value="users">المستخدمين (الخاص)</option></select><button class="act" onclick="broadcastAll()">📣 إرسال</button></div>'
...
async function broadcastAll(){
  ...
  const label=target==='groups'?'الجروبات':target==='channels'?'القنوات':target==='users'?'المستخدمين بالخاص':'الجروبات والقنوات';
